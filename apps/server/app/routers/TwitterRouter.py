...
async def GetCurrentTwitterAccount(
    screen_name: Annotated[
        str, Path(description="Twitter アカウントのスクリーンネーム。")
    ],
    ...
    twitter_account = await TwitterAccount.filter(
        user_id=current_user.id, screen_name=screen_name
    ).all()
    ...
    if len(twitter_account) == 0:
        logging.error(
            f"[TwitterRouter][GetCurrentTwitterAccount] TwitterAccount associated with screen_name does not exist [screen_name: {screen_name}]"
        )
        raise HTTPException(
...
async def TwitterPasswordAuthAPI(
    auth_request: Annotated[
        schemas.TwitterPasswordAuthRequest | schemas.TwitterCookieAuthRequest,
        Body(description="Twitter 認証リクエスト"),
    ],
    current_user: Annotated[User, Depends(GetCurrentUser)],
):
    """
    ...
    if isinstance(auth_request, schemas.TwitterPasswordAuthRequest):
        ...
        try:
            ...
            auth_handler = await asyncio.to_thread(
            ...
            if len(ex.api_codes) > 0 and len(ex.api_messages) > 0:
                error_message = (
                    f"Code: {ex.api_codes[0]} / Message: {ex.api_messages[0]}"
                )
            else:
                error_message = f"Unknown Error (HTTP Error {ex.response.status_code})"
            logging.error(
                f"[TwitterRouter][TwitterPasswordAuthAPI] Failed to authenticate with password ({error_message}) [screen_name: {auth_request.screen_name}]"
            )
            raise HTTPException(
            ...
            logging.error(
                f"[TwitterRouter][TwitterPasswordAuthAPI] Unexpected error occurred while authenticate with password ({error_message}) [screen_name: {auth_request.screen_name}]"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    # cookies.txt (Netscape 形式) をパースして Cookie を取得
        ...
        try:
            ...
            if not cookies:
                logging.error(
                    "[TwitterRouter][TwitterPasswordAuthAPI] No valid cookies found in the provided cookies.txt"
                )
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    try:
        ...
        logging.error(
            f"[TwitterRouter][TwitterPasswordAuthAPI] Failed to get user information for Twitter account @{twitter_account.screen_name}",
            exc_info=ex,
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    twitter_account.icon_url = verify_credentials.profile_image_url_https.replace(
        "_normal", ""
    )
    ...
    if existing_accounts:
        ...
        oldest_account.access_token_secret = (
        ...
        logging.info(
            f"[TwitterRouter][TwitterPasswordAuthAPI] Updated existing account and removed {len(existing_accounts) - 1} duplicate(s) [screen_name: {twitter_account.screen_name}]"
        )
    ...
    # 既存のアカウントが見つからなかった場合、新しいアカウント情報を DB に保存
        ...
        logging.info(
            f"[TwitterRouter][TwitterPasswordAuthAPI] Created new account [screen_name: {twitter_account.screen_name}]"
        )

    # 処理完了
    if isinstance(auth_request, schemas.TwitterCookieAuthRequest):
        logging.info(
            f"[TwitterRouter][TwitterPasswordAuthAPI] Logged in with cookie [screen_name: {twitter_account.screen_name}]"
        )
    else:
        logging.info(
            f"[TwitterRouter][TwitterPasswordAuthAPI] Logged in with password [screen_name: {twitter_account.screen_name}]"
        )
...
async def TwitterAccountDeleteAPI(
    ...
    assert twitter_account.access_token in [
        "COOKIE_SESSION",
        "DIRECT_COOKIE_SESSION",
    ], "OAuth session is no longer available."
    ...
    if twitter_account.access_token == "COOKIE_SESSION":
        ...
        try:
            ...
            logging.info(
                f"[TwitterRouter][TwitterAccountDeleteAPI] Logged out with password [screen_name: {twitter_account.screen_name}]"
            )
        except tweepy.HTTPException as ex:
            # サーバーエラーが発生した
            if len(ex.api_codes) > 0 and len(ex.api_messages) > 0:
                ...
                error_message = f"Unknown Error (HTTP Error {ex.response.status_code})"
            logging.error(
                f"[TwitterRouter][TwitterAccountDeleteAPI] Failed to logout ({error_message}) [screen_name: {twitter_account.screen_name}]"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            ...
            error_message = f"Message: {ex}"
            logging.error(
                f"[TwitterRouter][TwitterAccountDeleteAPI] Unexpected error occurred while logout ({error_message}) [screen_name: {twitter_account.screen_name}]"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
async def TwitterTweetAPI(
    ...
    tweet: Annotated[
        str,
        Form(
            description="ツイートの本文 (基本的には140文字までだが、プレミアムの加入状態や英数字の量に依存する) 。"
        ),
    ] = "",
    images: Annotated[
        list[UploadFile], File(description="ツイートに添付する画像 (4枚まで) 。")
    ] = [],
):
    """
    ...
    if len(images) > 4:
        logging.error(
            f"[TwitterRouter][TwitterTweetAPI] Can tweet up to 4 images [image length: {len(images)}]"
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    try:
        ...
        for image in images:
            image_upload_task.append(
                asyncio.to_thread(
                    tweepy_api.media_upload,
    ...
    # 画像のアップロードに失敗した
        ...
        if len(ex.api_codes) > 0 and len(ex.api_messages) > 0:
            ...
            error_message = (
                "ツイート画像のアップロードに失敗しました。"
                + TwitterGraphQLAPI.ERROR_MESSAGES.get(
                    ex.api_codes[0],
                    f"Code: {ex.api_codes[0]} / Message: {ex.api_messages[0]}",
                )
            )
            ...
            if len(ex.api_errors) > 0:
                error_message += (
                    f"Message: {ex.api_errors[0]}"  # エラーメッセージがあれば追加
                )
        return {
            "is_success": False,
            "detail": error_message,
    ...
    return await TwitterGraphQLAPI(twitter_account).createTweet(
        tweet, media_ids, request.headers.get("x-client-transaction-id")
    )
...
async def TwitterRetweetAPI(
    ...
    return await TwitterGraphQLAPI(twitter_account).createRetweet(
        tweet_id, request.headers.get("x-client-transaction-id")
    )
...
async def TwitterRetweetCancelAPI(
    ...
    return await TwitterGraphQLAPI(twitter_account).deleteRetweet(
        tweet_id, request.headers.get("x-client-transaction-id")
    )
...
async def TwitterFavoriteAPI(
    ...
    return await TwitterGraphQLAPI(twitter_account).favoriteTweet(
        tweet_id, request.headers.get("x-client-transaction-id")
    )
...
async def TwitterFavoriteCancelAPI(
    ...
    return await TwitterGraphQLAPI(twitter_account).unfavoriteTweet(
        tweet_id, request.headers.get("x-client-transaction-id")
    )
...
async def TwitterTimelineAPI(
    ...
    cursor_id: Annotated[
        str | None,
        Query(
            description="前回のレスポンスから取得した、次のページを取得するためのカーソル ID 。"
        ),
    ] = None,
):
    """
...
async def TwitterSearchAPI(
    ...
    search_type: Annotated[
        Literal["Top", "Latest"],
        Query(
            description="検索タイプ。Top は話題のツイート、Latest は最新のツイート。"
        ),
    ] = "Latest",
    cursor_id: Annotated[
        str | None,
