...
async def GetCurrentUser(
    ...
    current_user = (
        await User.filter(id=user_id).prefetch_related("twitter_accounts").get_or_none()
    )

    # そのユーザー ID のユーザーが存在しない
    if not current_user:
        logging.error(
            f"[GetCurrentUser] User associated with access token does not exist [user_id: {user_id}]"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def GetCurrentAdminUser(
    ...
    if current_user.is_admin is False:
        logging.error(
            f"[GetCurrentAdminUser] Don't have permission to access this resource [user_id: {current_user.id}]"
        )
        raise HTTPException(
...
async def GetSpecifiedUser(
    ...
    # 指定されたユーザー名のユーザーを取得
    user = (
        await User.filter(name=username)
        .prefetch_related("twitter_accounts")
        .get_or_none()
    )
    ...
    if not user:
        logging.error(
            f"[GetSpecifiedUser] Specified user was not found [username: {username}]"
        )
        raise HTTPException(
...
def ResizeAndSaveIcon(file: BinaryIO, save_path: pathlib.Path) -> None:
    ...
    # リサイズして保存
    pillow_image_resize = pillow_image_crop.resize(
        (RESIZE_WIDTH_AND_HEIGHT, RESIZE_WIDTH_AND_HEIGHT)
    )
...
async def UserCreateAPI(
    user_create_request: Annotated[
        ...
        Body(description="作成するユーザーの名前とパスワード。"),
    ],
):
    """
    新しいユーザーアカウントを作成する。
    ...
    if await User.filter(name=user_create_request.username).get_or_none() is not None:
        logging.error(
            f"[UsersRouter][UserCreateAPI] Specified username is duplicated [username: {user_create_request.username}]"
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    if user_create_request.username.lower() in PERMITTED_USERNAMES:
        logging.error(
            f"[UsersRouter][UserCreateAPI] Specified username is not permitted [username: {user_create_request.username}]"
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    current_user = await User.create(
        ...
        is_admin=(
            False if await User.all().count() > 0 else True
...
async def UserAccessTokenAPI(
    ...
    if not current_user:
        logging.error(
            f"[UsersRouter][UserAccessTokenAPI] Incorrect username [username: {form_data.username}]"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if not PASSWORD_CONTEXT.verify(form_data.password, current_user.password):
        logging.error(
            f"[UsersRouter][UserAccessTokenAPI] Incorrect password [username: {form_data.username}]"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def UserAPI(
    ...
    if await TwitterAccount.filter(icon_url="Temporary").count() > 0:
        ...
        current_user = (
            await User.filter(id=current_user.id)
            .prefetch_related("twitter_accounts")
            .get()
        )  # current_user のデータを更新
...
async def UserUpdateAPI(
    user_update_request: Annotated[
        schemas.UserUpdateRequest,
        Body(description="更新するユーザーアカウントの情報。"),
    ],
    current_user: Annotated[User, Depends(GetCurrentUser)],
):
    """
    ...
    if user_update_request.username is not None:
        ...
        if (
            user_update_request.username != current_user.name
            and await User.filter(name=user_update_request.username).get_or_none()
        ):
            logging.error(
                f"[UsersRouter][UserUpdateAPI] Specified username is duplicated [username: {user_update_request.username}]"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Specified username is duplicated",
        ...
        if user_update_request.username.lower() in PERMITTED_USERNAMES:
            logging.error(
                f"[UsersRouter][UserUpdateAPI] Specified username is not permitted [username: {user_update_request.username}]"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Specified username is not permitted",
    ...
    if user_update_request.password is not None:
        current_user.password = PASSWORD_CONTEXT.hash(
            user_update_request.password
        )  # ハッシュ化されたパスワード
...
async def UserUpdateIconAPI(
    image: Annotated[
        UploadFile, File(description="アカウントのアイコン画像 (JPEG or PNG)。")
    ],
    current_user: Annotated[User, Depends(GetCurrentUser)],
):
    """
    ...
    if image.content_type != "image/jpeg" and image.content_type != "image/png":
        logging.error(
            f"[UsersRouter][UserUpdateIconAPI] Please upload JPEG or PNG image [content_type: {image.content_type}]"
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    await asyncio.to_thread(
        ResizeAndSaveIcon, image.file, ACCOUNT_ICON_DIR / f"{current_user.id:02}.png"
    )
...
async def SpecifiedUserUpdateAPI(
    user_update_request: Annotated[
        schemas.UserUpdateRequestForAdmin,
        Body(description="更新するユーザーアカウントの情報。"),
    ],
    user: Annotated[User, Depends(GetSpecifiedUser)],
):
    ...
    if user_update_request.is_admin is False:
        ...
        if remaining_admins == 0:
            logging.error(
                "[UsersRouter][SpecifiedUserUpdateAPI] Cannot revoke admin permission because there are no more admins"
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
