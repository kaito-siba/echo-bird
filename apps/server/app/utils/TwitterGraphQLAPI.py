...
class TwitterGraphQLAPI:
    ...
    __challenge_info_cache: ClassVar[
        dict[str, tuple[float, schemas.TwitterChallengeData]]
    ] = {}

    # ツイートの最小送信間隔 (秒)
    MINIMUM_TWEET_INTERVAL = (
        20  # 必ずアカウントごとに 20 秒以上間隔を空けてツイートする
    )
    ...
    def __init__(self, twitter_account: TwitterAccount) -> None:
        ...
        self.graphql_headers_dict = cast(
            dict[str, str], self.cookie_session_user_handler.get_graphql_api_headers()
        )  # GraphQL API 用ヘッダー
        self.html_headers_dict = (
            self.cookie_session_user_handler.get_html_headers()
        )  # HTML 用ヘッダー
        ...
        try:
            ...
            for endpoint in endpoint_infos:
                ...
                if operation_name in cls.ENDPOINT_INFOS:
                    ...
                    cls.ENDPOINT_INFOS[operation_name] = (
                        schemas.TwitterGraphQLAPIEndpointInfo(
                            method=method,
                    ...
                    # 変更差分があるときのみ出力
                    if (
                        old_endpoint_info.query_id
                        != cls.ENDPOINT_INFOS[operation_name].query_id
                        or old_endpoint_info.method
                        != cls.ENDPOINT_INFOS[operation_name].method
                        or old_endpoint_info.features
                        != cls.ENDPOINT_INFOS[operation_name].features
                    ):
            ...
            logging.error(
                "Failed to update Twitter GraphQL API endpoint infos:", exc_info=ex
            )
    ...
    async def persistCookies(self) -> None:
        ...
        existing_cookies: dict[str, str] = json.loads(
            self.twitter_account.access_token_secret
        )
        ...
        self.twitter_account.access_token_secret = json.dumps(
            existing_cookies, ensure_ascii=False
        )
        await self.twitter_account.save()

    async def fetchChallengeData(
        self,
    ) -> schemas.TwitterChallengeData | schemas.TwitterAPIResult:
        """
        ...
        if self.twitter_account.screen_name in self.__challenge_info_cache:
            cached_time, cached_challenge_data = self.__challenge_info_cache[
                self.twitter_account.screen_name
            ]
        ...
        twitter_web_app_html = await self.httpx_client.get(
            "https://x.com/home", headers=self.html_headers_dict
        )
        ...
        if meta_tag is None:
            logging.error(
                "[TwitterGraphQLAPI] Failed to fetch verification code from Twitter Web App HTML"
            )
            return schemas.TwitterAPIResult(
                is_success=False,
        ...
        # HTML からチャレンジコードを取得
        challenge_code_match = re.search(
            r'"ondemand.s":"(\w+)"', twitter_web_app_html_text
        )
        if not challenge_code_match:
            logging.error(
                "[TwitterGraphQLAPI] Failed to fetch challenge code from Twitter Web App HTML"
            )
            return schemas.TwitterAPIResult(
                is_success=False,
        ...
        challenge_animation_svg_codes = [
            str(svg) for svg in soup.select('svg[id^="loading-x"]')
        ]
        ...
        self.__challenge_info_cache[self.twitter_account.screen_name] = (
            time.time(),
            challenge_data,
        )
    ...
    async def invokeGraphQLAPI(
        ...
        if x_client_transaction_id is not None:
            ...
            logging.debug_simple(
                f"[TwitterGraphQLAPI][{endpoint_info.endpoint}] X-Client-Transaction-ID: {x_client_transaction_id}"
            )
        ...
        if endpoint_info.endpoint in [
            ...
            headers["authorization"] = (
                self.cookie_session_user_handler.TWEETDECK_BEARER_TOKEN
            )
            ...
            if endpoint_info.method == "POST":
                ...
                if endpoint_info.features is not None:
                    params = {
                        ...
                        "features": json.dumps(
                            endpoint_info.features, ensure_ascii=False
                        ),
                    }
                else:
                    params = {
        ...
        # 接続エラー（サーバーメンテナンスやタイムアウトなど）
            ...
            logging.error(
                "[TwitterGraphQLAPI] Failed to connect to Twitter GraphQL API"
            )
        ...
        if not (200 <= response.status_code < 300):
            logging.error(
                f"[TwitterGraphQLAPI] Failed to invoke GraphQL API (HTTP Error {response.status_code})"
            )
            logging.error(f"[TwitterGraphQLAPI] Response: {response.text}")
            return (
                error_message_prefix
                + f"Twitter API から HTTP {response.status_code} エラーが返されました。"
            )
        ...
        try:
            ...
            return (
                error_message_prefix
                + "Twitter API のレスポンスを JSON としてパースできませんでした。"
            )
        ...
        if "errors" in response_json and "data" not in response_json:
            ...
            alternative_error_message = (
                f"Code: {response_error_code} / Message: {response_error_message}"
            )
            logging.error(
                f"[TwitterGraphQLAPI] Failed to invoke GraphQL API ({alternative_error_message})"
            )

            # エラーコードに対応するエラーメッセージを返し、対応するものがない場合は alternative_error_message を返す
            return error_message_prefix + self.ERROR_MESSAGES.get(
                response_error_code, alternative_error_message
            )
    ...
    async def createTweet(
        ...
        if screen_name not in self.__tweet_locks:
            self.__tweet_locks[screen_name] = _TweetLockInfo(
                lock=asyncio.Lock(), last_tweet_time=0.0
            )
        ...
        async with self.__tweet_locks[screen_name]["lock"]:
            ...
            wait_time = max(
                0, self.MINIMUM_TWEET_INTERVAL - (current_time - last_tweet_time)
            )
            if wait_time > 0:
            ...
            try:
                tweet_id = str(
                    response["create_tweet"]["tweet_results"]["result"]["rest_id"]
                )
            except Exception:
    ...
    async def createRetweet(
        self, tweet_id: str, x_client_transaction_id: str | None = None
    ) -> schemas.TwitterAPIResult:
        """
        ツイートをリツイートする
    ...
    def __getCursorIDFromTimelineAPIResponse(
        ...
        # それ以外のレスポンス (通常あり得ないため、ここに到達した場合はレスポンス構造が変わった可能性が高い)
            ...
            instructions = []
            logging.warning(
                f"[TwitterGraphQLAPI] Unknown timeline response format: {response}"
            )
    ...
    def __getTweetsFromTimelineAPIResponse(
        ...
        def format_tweet(raw_tweet_object: dict[str, Any]) -> schemas.Tweet:
            ...
            if "retweeted_status_result" in raw_tweet_object["legacy"]:
                retweeted_tweet = format_tweet(
                    raw_tweet_object["legacy"]["retweeted_status_result"]["result"]
                )
            ...
            if "quoted_status_result" in raw_tweet_object:
                if "result" not in raw_tweet_object["quoted_status_result"]:
                    ...
                    quoted_tweet = format_tweet(
                        raw_tweet_object["quoted_status_result"]["result"]
                    )
                ...
                for media in raw_tweet_object["legacy"]["extended_entities"]["media"]:
                    if media["type"] == "photo":
                        ...
                        mp4_variants: list[dict[str, Any]] = list(
                            filter(
                                lambda variant: variant["content_type"] == "video/mp4",
                        ...
                        if len(mp4_variants) > 0:
                            ...
                            movie_url = (
                                str(highest_bitrate_variant["url"])
                                if "url" in highest_bitrate_variant
                                else None
                            )
            ...
            if (
                "entities" in raw_tweet_object["legacy"]
                and "urls" in raw_tweet_object["legacy"]["entities"]
            ):
                for url_entity in raw_tweet_object["legacy"]["entities"]["urls"]:
                    if (
                        "expanded_url" in url_entity
                    ):  # 展開後の URL が存在する場合のみ (稀に存在しない場合がある)
                        expanded_text = expanded_text.replace(
                            url_entity["url"], url_entity["expanded_url"]
                        )
            ...
            return schemas.Tweet(
                ...
                user=schemas.TweetUser(
                    ...
                    name=raw_tweet_object["core"]["user_results"]["result"]["core"][
                        "name"
                    ],
                    screen_name=raw_tweet_object["core"]["user_results"]["result"][
                        "core"
                    ]["screen_name"],
                    # (ランダムな文字列)_normal.jpg だと画像サイズが小さいので、(ランダムな文字列).jpg に置換
                    icon_url=raw_tweet_object["core"]["user_results"]["result"][
                        "avatar"
            ...
            if instruction.get("type") == "TimelineAddEntries":
                ...
                for entry in entries:
                    ...
                    if (
                        ...
                        and content.get("itemContent", {}).get("itemType")
                        == "TimelineTweet"
                    ):
                        tweet_results = (
                        ...
                        if tweet_results and tweet_results.get("__typename") in [
                            "Tweet",
    ...
    async def homeLatestTimeline(
        ...
        if next_cursor_id is None or previous_cursor_id is None:
            logging.error(
                "[TwitterGraphQLAPI] Failed to fetch timeline: Cursor ID not found"
            )
            return schemas.TwitterAPIResult(
                is_success=False,
    ...
    async def searchTimeline(
        ...
        variables["querySource"] = (
            ""  # Twitter Web App 自体が空文字を送っているので合わせる
        )
        ...
        if next_cursor_id is None or previous_cursor_id is None:
            logging.error(
                "[TwitterGraphQLAPI] Failed to search tweets: Cursor ID not found"
            )
            return schemas.TwitterAPIResult(
                is_success=False,
