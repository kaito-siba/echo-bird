    ...
    async def updateAccountsInformation(cls):
        ...
        for twitter_account in await TwitterAccount.all():
            ...
            try:
                ...
                logging.error(
                    f"Failed to get user information for Twitter account @{twitter_account.screen_name}",
                    exc_info=ex,
                )
                continue
            # アカウント名
            ...
            twitter_account.icon_url = (
                verify_credentials.profile_image_url_https.replace("_normal", "")
            )
        ...
        logging.info(
            f"Twitter accounts update complete. ({round(time.time() - timestamp, 3)} sec)"
        )
