...
class User(TortoiseModel):
    ...
    niconico_user_premium = cast(
        TortoiseField[bool | None], fields.BooleanField(null=True)
    )
    niconico_access_token = cast(TortoiseField[str | None], fields.TextField(null=True))
    niconico_refresh_token = cast(
        TortoiseField[str | None], fields.TextField(null=True)
    )
    ...
    async def refreshNiconicoAccessToken(self) -> None:
        ...
        try:
            ...
            if token_api_response.status_code != 200:
                ...
                try:
                    ...
                    pass
                raise Exception(
                    f"アクセストークンの更新に失敗しました。(HTTP Error {token_api_response.status_code}{error_code})"
                )
        ...
        try:
            ...
            ## 3秒応答がなかったらタイムアウト
            user_api_url = (
                f"https://nvapi.nicovideo.jp/v1/users/{self.niconico_user_id}"
            )
            async with HTTPX_CLIENT() as client:
                # X-Frontend-Id がないと INVALID_PARAMETER になる
                user_api_response = await client.get(
                    user_api_url, headers={**API_REQUEST_HEADERS, "X-Frontend-Id": "6"}
                )

            if user_api_response.status_code == 200:
                # ユーザー名
                self.niconico_user_name = str(
                    user_api_response.json()["data"]["user"]["nickname"]
                )
                # プレミアム会員かどうか
                self.niconico_user_premium = bool(
                    user_api_response.json()["data"]["user"]["isPremium"]
                )
